from api.controllers.auth_controller import Authontroller
...
class Factory:
    ...
    def create_person_router(database_manager):
        ...
        return PersonRouter(PersonDataConstStrings.person, person_controller)
    
    @staticmethod
    def create_table_router(database_manager):
        table_controller = TableController(database_manager)
        return TableRouter(TableDataConstStrings.table, table_controller)
    
    @staticmethod
    def create_auth_router(database_manager):
        auth_controller = AuthController(database_manager)
        return AuthRouter(AuthDataConstStrings.auth, auth_controller)
    ...
    def create_zmq_server(routers):
        return ZMQServerManager(routers, ConstStrings.localhost, Consts.port)
